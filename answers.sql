...
SELECT models.name AS model_name, brands.name AS brand_name, brands.headquarters FROM models JOIN brands USING (brand_id) WHERE brands.name = 'Ford' AND models.name = 'Mustang';
...
SELECT models.year, models.name, models.model_id, awards.name FROM models LEFT JOIN awards ON models.model_id = awards.winner_id WHERE models.year > 1963;
...
FROM brands AS b LEFT JOIN models AS m ON b.brand_id = m.brand_id
...
FROM models AS m LEFT JOIN brands AS b ON m.brand_id = b.brand_id
...
SELECT brands.name, brands.founded FROM brands LEFT JOIN models USING (brand_id) WHERE brands.name = 'Tesla';
